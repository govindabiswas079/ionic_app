...
const TotalOrder = ({ refreshControll }) => {
  ...
  React.useEffect(async () => {
    ...
    axios.all([
      ApiRequest.post(sanitizeUrl(`/vendorordergraph`), {
        vendor_id: vendor_id
      }, { signal: signal }),
      ...
      .then(axios.spread((data1, data2) => {
        if (data1.data.response.status === 1) {
          setVendorDashboardinfoIncom(data1.data.orderGraph)
          setLoader(false)
        } else {
          if (data1.data.response.status === -1) {
            setLoader(false)
                  ...
                  <Grid container alignItems="center" justifyContent="space-between">
                    ...
                    <Grid item>
                      <Typography variant="h4" sx={{ color: theme.palette.grey[800], fontFamily: 'Nunito', fontWeight: '700' }}>
                        {vendorDashboardinfo.order_success === undefined ? '0' : vendorDashboardinfo?.order_success}
                      </Typography>
                    </Grid>
                  </Grid>
                </Grid>
